...
args = default_parse([add_trainer_args, add_checkpoint_args , TorchVisionDataSet.add_model_specific_args, ClassificationModel.add_model_specific_args, WideResNet.add_model_specific_args, SWACallback.add_model_specific_args])
args_trainer = args
args = vars(args)
...
args["lr_decay_cosine_steps"] = args["swa_epoch_start"]
# args["annealing_steps"] = len(dataset.train_dataloader()) * args["annealing_epochs"] # dont need that for now
neural_network = WideResNet(**args)
model = ClassificationModel(model=neural_network, **args)

callbacks = setup_callbacks(root_dir, callbacks=[], **args)
callbacks.append(SWACallback(**args))
